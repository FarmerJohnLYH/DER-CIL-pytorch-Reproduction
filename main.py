...
base_dir = osp.relpath(osp.dirname(osp.abspath(__file__)), repo_name)
# base_dir = osp.realpath(".")[:osp.realpath(".").index(repo_name) + len(repo_name)]

sys.path.insert(0, base_dir) # 添加项目根目录到环境变量
...
# MongoDB Observer
# ex.observers.append(MongoObserver.create(url='xx.xx.xx.xx:port', db_name='classil')) # MongoDB Observer
...
def initialization(config, seed, mode, exp_id):
    ...
    torch.backends.cudnn.benchmark = True  # 这将导致非确定性结果。
    # ex.captured_out_filter = lambda text: 'Output capturing turned off.'
    cfg = edict(config) # 将config转换为edict
    ...
    # tensorboard 用于记录训练过程中的数据，如loss,acc等
    # 存放于./tensorboard/
    # 
...
def _train(cfg, _run, ex, tensorboard):
    ...
    ex.logger.info(inc_dataset.class_order) #  class_order?
    ...
    for task_i in range(inc_dataset.n_tasks):
        ...
        if task_i == 0 and cfg["start_class"] > 0:
            ...
            model.train_task(train_loader, val_loader) # 训练的 main function
...
if __name__ == "__main__":
    ex.add_config("./configs/lyh2.yaml") # 添加配置文件
    #lyh 修改
    ex.run_commandline()
