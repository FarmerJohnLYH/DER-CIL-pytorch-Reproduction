...
class MemorySize:
    ...
    def update_memory_per_cls(self, network, n_classes, task_size): 
        # cls 是 class 的缩写
        # mem_size_mode: "uniform_fixed_total_mem" #uniform_fixed_per_cls,
        # uniform_fixed_total_mem, dynamic_fixed_per_cls
        if "uniform" in self.mode:
            self.update_memory_per_cls_uniform(n_classes)
        else:
            # 调整 memory 的次数不同 
            if n_classes == task_size: # task_size 是每次增加的类别数 = ['increment'] = 5
                self.update_memory_per_cls_uniform(n_classes)
