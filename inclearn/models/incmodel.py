...
class IncModel(IncrementalLearner):
    def __init__(self, cfg, trial_i, _run, ex, tensorboard, inc_dataset):
        ...
        # Optimizer paras
        # Optimizer paras
        self._opt_name = cfg["optimizer"]  # "sgd"
        self._warmup = cfg['warmup']  # Not explicitly defined in provided config
        self._lr = cfg["lr"]  # 0.1
        self._weight_decay = cfg["weight_decay"]  # 0.0005
        self._n_epochs = cfg["epochs"]  # 70
        self._scheduling = cfg["scheduling"]  # [48, 63]
        self._lr_decay = cfg["lr_decay"]  # 0.1
        ...
        self._parallel_network = DataParallel(self._network) # 用于多GPU训练
        # DataParallel 是 PyTorch 中的一个类，用于自动并行化数据处理和模型计算。
        # 当你将模型传递给 DataParallel 类的实例时，PyTorch 会自动将输入的数据分割成多个小批次，并将每个小批次的数据发送到不同的 GPU 上进行处理。
        # 然后，DataParallel 会收集各个 GPU 上的输出结果，并将它们合并起来，以便进行下一步的处理或计算损失。
        self._train_head = cfg["train_head"]
        self._infer_head = cfg["infer_head"]
        self._old_model = None # 用于存储旧模型
    ...
    def train(self):
        # 核心代码
        if self._der:  # 如果启用了 DER 模式（动态可扩展表示）
            self._parallel_network.train()  # 将整个并行网络设置为训练模式
            self._parallel_network.module.convnets[-1].train()  # 将最新添加的卷积网络模块设置为训练模式
            if self._task >= 1:  # 如果当前任务编号大于等于1
                for i in range(self._task):  # 遍历所有之前的任务
                    self._parallel_network.module.convnets[i].eval()  
                    #【重点】 将之前任务的卷积网络模块设置为评估模式（冻结参数）
        else:  # 如果没有启用 DER 模式
            self._parallel_network.train()  # 将整个并行网络设置为训练模式
    ...
    def _train_task(self, train_loader, val_loader): # 真正的训练
        self._ex.logger.info(f"nb {len(train_loader.dataset)}")

        topk = 5 if self._n_classes > 5 else self._task_size
        accu = ClassErrorMeter(accuracy=True, topk=[1, topk]) # 计算分类准确率
    ...
    def _compute_loss(self, inputs, targets, outputs, old_classes, new_classes):
        loss = F.cross_entropy(outputs['logit'], targets) # 计算交叉熵损失

        if outputs['aux_logit'] is not None:
            aux_targets = targets.clone()
            if self._cfg["aux_n+1"]:
                aux_targets[old_classes] = 0
                aux_targets[new_classes] -= sum(self._inc_dataset.increments[:self._task]) - 1 # 意思是减去之前的类别数
            aux_loss = F.cross_entropy(outputs['aux_logit'], aux_targets) # 计算辅助损失，计算方式为交叉熵
        else:
            aux_loss = torch.zeros([1]).cuda() # 如果没有辅助损失，则设置为0
